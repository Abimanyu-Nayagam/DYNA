...
const EditValorantPortfolioPage = () => {
    ...
    const loadExistingProfile = async () => {
        try {
            const response = await valorantAPI.getMyProfile();
            ...
            setFormData({
                ...
                region: profile.region || 'Asia-Pacific',
                server: profile.server || 'Mumbai',
                started_playing: profile.started_playing || '',
                current_rank: profile.current_rank || 'Iron 1',
                current_season: profile.current_act?.season || 'EP09',
                current_act_number: profile.current_act?.act_number || 1,
                peak_rank: profile.peak_rank || 'Iron 1',
                peak_season: profile.peak_act?.season || 'EP09',
                peak_act_number: profile.peak_act?.act_number || 1,
                peak_rank_date: profile.peak_act?.date || '',
                kd: profile.kd || '',
                win_rate: profile.win_rate || '',
                total_matches: profile.total_matches || '',
                hours_played: profile.hours_played || '',
                main_role: profile.main_role || 'Duelist',
                playstyle_description: profile.playstyle_description || '',
                aggressiveness: profile.aggressiveness || 5,
                utility_usage: profile.utility_usage || 5,
                anchoring_skill: profile.anchoring_skill || 5,
                lurking_skill: profile.lurking_skill || 5,
                entry_confidence: profile.entry_confidence || 5,
                best_agent: profile.best_agent || 'Jett',
                top_agents: profile.top_agents || [],
                in_team: profile.in_team || false,
                current_team: profile.current_team || '',
                role_in_team: profile.role_in_team || '',
                been_in_team_before: profile.been_in_team_before || false,
                bio: profile.bio || '',
                is_public: profile.is_public || false
            });

            // Populate team history
            if (profile.team_history && profile.team_history.length > 0) {
                setTeamHistory(profile.team_history.map((t: any) => ({
            ...
            if (profile.tournaments && profile.tournaments.length > 0) {
                setTournaments(profile.tournaments.map((t: any) => ({
                    name: t.name || '',
                    organizer: t.organizer || '',
                    year: t.year || new Date().getFullYear(),
                    placement: t.placement || '',
                    role_in_tournament: t.role_in_tournament || '',
                    notes: t.notes || ''
                })));
            }
        ...
        <div className="valo-create-page">
            <div className="valo-page-container">
                <div className="valo-page-header">
                    <h1>Edit Valorant Portfolio</h1>
                    <button className="valo-back-btn" onClick={() => navigate('/players/valorant/me')}>
                        ← Back to Profile
                    </button>
                </div>
...
export default EditValorantPortfolioPage;